    ...
    <Panel label="🌙 야간 갭, 다음날 시가를 맞췄나">
      ...
      <div className="flex gap-2 mb-3">
        <div className="flex-1 bg-surface-2 rounded-lg px-2.5 py-2">
          <div className="text-label text-muted">방향 일치율</div>
          <div className="font-mono text-base font-semibold">
            {(data.directionMatch ?? 0).toFixed(0)}%
          </div>
        </div>
        <div className="flex-1 bg-surface-2 rounded-lg px-2.5 py-2">
          <div className="text-label text-muted">상관계수</div>
          <div className="font-mono text-base font-semibold">
      ...
      <div className="space-y-1">
        <div className="flex text-label text-muted pb-1 border-b border-border">
          <span className="flex-1">야간 갭이</span>
          <span className="w-16 text-right">다음날 시가</span>
          <span className="w-14 text-right">상승비율</span>
        </div>
        {data.buckets.map((b) => (
          <div key={b.label} className="flex items-center text-label py-0.5">
            <span className="flex-1">
              {b.label}
              <span className="text-muted text-label ml-1">n={b.count}</span>
            </span>
      ...
      <p className="text-label text-muted leading-relaxed mt-3">
