...
function Bar({ label, value, max, tone }: { label: string; value: number; max: number; tone: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-label text-muted w-[4.5rem] shrink-0">{label}</span>
      <div className="flex-1 h-2 rounded-full bg-surface-2 overflow-hidden">
        <div className={`h-full rounded-full ${tone}`} style={{ width: `${w}%` }} />
      </div>
      <span className={`font-mono text-label tnum w-[3.8rem] text-right ${changeColor(value)}`}>
        {pct(value)}
      </span>
    </div>
  )
  ...
  return (
    <Panel label={`⚠️ 레버리지 감쇠 · ${data.leverage}배 상품`}>
      ...
      {worseThanUnleveraged && (
        <p className="text-label text-down leading-relaxed mt-3 bg-down/10 border border-down/30 rounded-lg px-2.5 py-2">
