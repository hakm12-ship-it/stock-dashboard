...
- 정의: `미래PER(t) = 현재가 ÷ 예상EPS(t년 후)`, t = 1/2/3년. **현재 PER과 나란히** 표시해 "시간이 갈수록 싸지는지" 확인.
- 예상 EPS 출처: **yfinance 애널리스트 컨센서스**(`Ticker.earnings_estimate` → 올해·내년 avg EPS). → 보통 **1~2년치만 신뢰성 있음.**
- **3년치**: 컨센서스 없으면 **예상 성장률(애널리스트 growth 또는 과거 EPS CAGR)로 연장한 추정** → 화면에 **"성장가정 추정"** 라벨 명시.
- ⚠️ 추정치는 전망·가정에 따라 크게 변동. 투자조언 아님 문구 유지.
...
- 삼성/하이닉스 상세: 밸류에이션에 **미래 PER 1/2/3년 행** 추가.
