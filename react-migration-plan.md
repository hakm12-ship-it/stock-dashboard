...
### 모바일 우선 (Mobile-first) — 최우선 제약
- **폰 세로 화면 기준으로 먼저 설계**, 데스크톱은 넓혀서 대응(반대 아님).
- 터치 타겟 **최소 44px**, 한 손 조작 고려. 탭·칩·버튼 넉넉하게.
- **하단 탭바**(네이티브 앱 느낌: 종합신호·기술·기본·뉴스) 검토 — 상단 탭보다 엄지 접근성↑.
- 지수 스트립·3종목 전환은 **가로 스크롤 칩** 또는 컴팩트 배치.
- 차트: 폰 폭에 맞춘 높이·캔들 밀도, **핀치 줌/스와이프** 지원(lightweight-charts 기본 제공). 기본 기간 짧게(3개월).
- 세로 스크롤 흐름 하나로, 좌우 스크롤 금지(표는 자체 스크롤 컨테이너).
- 폰트·여백은 작은 화면에서 가독성 우선. Tailwind는 base=모바일, `md:`↑에서 데스크톱 확장.
